              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    console.log('✅ Title found:', formData.title)
                    setIsLoading(true)
                    setError('')
                    try {
                      console.log('📦 Calling server /api/generate endpoint...')
                      const res = await fetch('/api/generate', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ title: formData.title, theme: formData.theme, eventType: formData.eventType })
                      })
                      ...
                      if (!res.ok) {
                        ...
                        throw new Error(data?.error || 'Server failed to generate description')
                      }
                      console.log('✅ Description received from server')
                  ...
                  className="flex items-center gap-1 px-3 py-1 text-xs bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-md hover:from-purple-700 hover:to-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                >
